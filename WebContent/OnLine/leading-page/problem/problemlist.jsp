...
<head>
    ...
    <link rel="stylesheet" href="../../css/problem/problemlist.css}" />
    ...
    <script type="text/javascript" src="../../js/problem/problemlist.js"></script>
</head>
<body>
<div class="ui fixed inverted menu">
    ...
    <a href="problemset.html" class="active item">
...
<div class="ui header container">
    <div class="ui large breadcrumb">
        ...
        <a class="section" href="problemset.html">题库中心</a>
        <i class="right chevron icon divider"></i>
        <div class="active section" text="${data['subject'].name}">数据结构与算法</div>
    </div>
</div>
...
<div class="ui problemList container">
    ...
    <div class="ui accordion segment">
        ...
        <div class="title active"><i class="dropdown icon"></i>筛选</div>
        <!-- 搜索区域 -->
    ...
    <table class="ui table">
        ...
        <tr each="item,itemStats : ${data['questions']}">
            <td><span text="${item.title}">一个作业的完成要经过若干加工步骤,每个步骤称为( )</span></td>
            <td>
                <div if="${item.difficulty == 1}" class="ui star rating" data-rating="1"></div>
                <div if="${item.difficulty == 2}" class="ui star rating" data-rating="2"></div>
                <div if="${item.difficulty == 3}" class="ui star rating" data-rating="3"></div>
                <div if="${item.difficulty == 4}" class="ui star rating" data-rating="4"></div>
                <div if="${item.difficulty == 5}" class="ui star rating" data-rating="5"></div>
            </td>
            <td>
                <span if="${item.questionType == 0}">单选题</span>
               <!-- <span if="${item.questionType == 1}">多选题</span>
                <span if="${item.questionType == 2}">问答题</span>
                <span if="${item.questionType == 3}">编程题</span>-->
            </td>
            <td><a href="problemdetail.html" class="tiny blue ui button">详情</a></td>
        </tr>
        <tr each="item,itemStats : ${data['questions']}">
            <td><span text="${item.title}">一个作业的完成要经过若干加工步骤,每个步骤称为2( )</span></td>
            <td>
                <div if="${item.difficulty == 1}" class="ui star rating" data-rating="1"></div>
                <div if="${item.difficulty == 2}" class="ui star rating" data-rating="2"></div>
                <div if="${item.difficulty == 3}" class="ui star rating" data-rating="3"></div>
